...
namespace BuildSmart.Maui.Services
{
		...
		public async Task<string?> AuthenticateWithFacebookAsync()
		{
            var authResult = await Microsoft.Maui.Authentication.WebAuthenticator.Default.AuthenticateAsync(
                new Uri($"{BuildSmart.SharedUI.ApiConfig.GetBaseUrl()}/api/externalauth/facebook-login?returnUrl=buildsmart://auth"),
                new Uri("buildsmart://"));
