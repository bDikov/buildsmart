...
public class NavigationBridge : INavigationBridge
{
    ...
    public Task NavigateToAsync(string route, IDictionary<string, object>? parameters = null)
    {
        MainThread.BeginInvokeOnMainThread(async () =>
        {
            ...
            // Append parameters to query string if this is intended for Blazor
            bool isBlazorRoute = (blazorUrl.StartsWith("/") && !blazorUrl.StartsWith("//")) || blazorUrl.StartsWith("http");

            if (isBlazorRoute && parameters != null && parameters.Count > 0)
            {
            ...
            if (blazorUrl.StartsWith("/") && !blazorUrl.StartsWith("//"))
            {
                ...
                {
                    // Store the target route and launch the Blazor WebView host page
                    _blazorNavigationRegistry.PendingRoute = blazorUrl;
            ...
            if (parameters != null)
            {
                await Shell.Current.GoToAsync(blazorUrl, parameters);
            }
            else
            {
                await Shell.Current.GoToAsync(blazorUrl);
            }
        });
