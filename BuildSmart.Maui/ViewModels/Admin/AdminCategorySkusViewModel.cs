...
public partial class AdminCategorySkusViewModel : ObservableObject, IQueryAttributable
{
    ...
    public void ApplyQueryAttributes(IDictionary<string, object> query)
    {
        if (query.TryGetValue("categoryId", out var idObj) && idObj != null)
        {
            ...
            MainThread.BeginInvokeOnMainThread(async () => await LoadSkusAsync());
        }
    }
    ...
    public async Task SaveNewSkuAsync()
    {
        ...
        {
            await Shell.Current.DisplayAlert("Validation Error", "SKU Code and Name are required.", "OK");
            return;
        }

        if (!decimal.TryParse(NewSkuBasePrice, out var price))
        {
            await Shell.Current.DisplayAlert("Validation Error", "Base Price must be a valid number.", "OK");
            return;
        }
    ...
    {
        bool confirm = await Shell.Current.DisplayAlert("Confirm", "Delete this SKU?", "Yes", "No");
        if (!confirm) return;
