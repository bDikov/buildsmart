...
public partial class UserProfileViewModel : ObservableObject
{
    ...
    private async Task ChangePhotoAsync()
    {
        ...
        {
            await AppServiceLocator.Alerts.DisplayAlert("Error", $"Failed to pick photo: {ex.Message}", "OK");
        }
    }
    ...
    private async Task AddPortfolioItemAsync()
    {
        try
        {
            ...
            string title = await AppServiceLocator.Alerts.DisplayPromptAsync("Portfolio", "Enter a title for this work:", "Upload", "Cancel", "Kitchen Renovation...");
            ...
            if (result != null)
            {
                await AppServiceLocator.Alerts.DisplayAlert("Success", "Portfolio item added.", "OK");
                await LoadProfileAsync();
            }
            else
            {
                await AppServiceLocator.Alerts.DisplayAlert("Error", "Failed to upload portfolio item.", "OK");
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task AddCertificationAsync()
    {
        try
        {
            ...
            string title = await AppServiceLocator.Alerts.DisplayPromptAsync("Certification", "Enter certificate name:", "Next", "Cancel", "Master Plumber License...");
            if (string.IsNullOrWhiteSpace(title)) return;
            ...
            if (result != null)
            {
                await AppServiceLocator.Alerts.DisplayAlert("Success", "Certification added.", "OK");
                await LoadProfileAsync();
            }
            else
            {
                await AppServiceLocator.Alerts.DisplayAlert("Error", "Failed to upload certification.", "OK");
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task ChangeVideoIntroAsync()
    {
        try
        {
            ...
            if (result != null)
            {
                await AppServiceLocator.Alerts.DisplayAlert("Success", "Video introduction updated.", "OK");
                await LoadProfileAsync();
            }
            else
            {
                await AppServiceLocator.Alerts.DisplayAlert("Error", "Failed to upload video.", "OK");
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task SaveProfileAsync()
    {
        ...
        try
        {
            ...
            await AppServiceLocator.Alerts.DisplayAlert("Success", "Profile updated successfully.", "OK");
        }
        catch (Exception ex)
        {
            await AppServiceLocator.Alerts.DisplayAlert("Error", ex.Message, "OK");
