...
public partial class AdminJobReviewViewModel : ObservableObject
{
	...
	[ObservableProperty]
	private ObservableCollection<IJobTaskDetails> _adminJobTasks = new();
	...
	private async Task ViewJobDetails(IGetProjectsForReview_ProjectsForReview_JobPosts? job)
	{
		...
		// Fetch the homeowner's task breakdown via lazy loading
		try
		{
			IsTasksLoading = true;
			var result = await _apiClient.GetJobTasks.ExecuteAsync(job.Id);
			var detailedJobPost = result.Data?.AllJobPosts?.FirstOrDefault();
			if (detailedJobPost?.JobTasks != null)
			{
				foreach (var task in detailedJobPost.JobTasks.OrderBy(t => t.SequenceOrder))
				{
					AdminJobTasks.Add((IJobTaskDetails)task);
				}
			}
		}
		catch (Exception ex)
		{
			System.Diagnostics.Debug.WriteLine($"Error fetching Admin Job Tasks: {ex.Message}");
		}
		finally
		{
	...
	private async Task ResolveFeedbackAsync(Guid feedbackId)
	{
		try
		{
			...
			await LoadJobsAsync();
			var updatedProject = Projects.FirstOrDefault(p => p.JobPosts.Any(j => j.Feedbacks.Any(f => f.Id.ToString() == feedbackId.ToString())));
			var updatedJob = updatedProject?.JobPosts.FirstOrDefault(j => j.Feedbacks.Any(f => f.Id.ToString() == feedbackId.ToString()));
			if (updatedJob != null) ViewJobDetails(updatedJob);
		}
		catch (Exception ex)
		{
	...
	private async Task PerformReview(Guid jobId, bool approved, string? feedback)
	{
		try
		{
			...
			await AppServiceLocator.Alerts.DisplayAlert("Success", approved ? "Job published to tradesmen." : "Job rejected and feedback sent.", "OK");
