...
public class GeminiAiService : IAiService
{
	...
	public async Task<AiTaskPricingResponse> CalculateTaskPricesAsync(List<JobTask> tasks, List<ServiceSku> allowedSkus, string humanReadableContext, string languageCode = "bg", CancellationToken cancellationToken = default)
	{
		try
		{
			...
			prompt.AppendLine();
			prompt.AppendLine("1. --- GENERAL & OVERHEAD ---");
			prompt.AppendLine("   - Site Protection / Logistics: Quantity = 1 (Fixed). Map if task involves 'подготовка', 'защита', 'покриване'.");
			prompt.AppendLine("   - Final Waste Removal: Quantity = ID: global_total_sqm. Map if task involves 'почистване', 'извозване'.");
			prompt.AppendLine();
			prompt.AppendLine("2. --- ELECTRICAL (ELEC) ---");
			prompt.AppendLine("   - Cable Laying (ELEC-CABLE-LAY) & Tubes (ELEC-LAY-TUBE):");
			prompt.AppendLine("       * If ID: elec_scope contains 'Цялостна подмяна', Quantity = (ID: global_total_sqm * 3.5).");
			prompt.AppendLine("       * If ID: elec_scope contains 'Частичен ремонт', Quantity = (ID: global_total_sqm * 1.0).");
			prompt.AppendLine("       * If ID: elec_scope contains 'Само монтаж', Quantity = 0 (Do not map cable/tubes).");
			prompt.AppendLine("   - Chasing (ELEC-CHASE-CONC or ELEC-CHASE-BRICK based on ID: elec_walls): Same quantity logic as Cable Laying. If Walls are 'Гипсокартон', quantity is 0.");
			prompt.AppendLine("   - Heavy Duty Cable (ELEC-CABLE-HEAVY): Quantity = (Count of items in ID: elec_heavy_appliances + ID: elec_ac_count) * 10 meters.");
			prompt.AppendLine("   - Standard Points (ELEC-POINT-STD):");
			prompt.AppendLine("       * If ID: elec_outlets_comfort is 'Базово', Quantity = (ID: global_room_count * 3) + 4.");
			prompt.AppendLine("       * If ID: elec_outlets_comfort is 'Комфорт', Quantity = (ID: global_room_count * 5) + 6.");
			prompt.AppendLine("       * If ID: elec_outlets_comfort is 'Премиум', Quantity = (ID: global_room_count * 8) + 10.");
			prompt.AppendLine("   - Panel Niche (ELEC-PANEL-NICHE): Quantity = 1 ONLY if ID: elec_panel contains 'скрито (вградено)'.");
			prompt.AppendLine();
			prompt.AppendLine("3. --- PLUMBING (PLMB) ---");
			prompt.AppendLine("   - Water Points (PLMB-POINT-NEW):");
			prompt.AppendLine("       * If ID: plumb_scope contains 'Цялостна подмяна', Quantity = (ID: global_bathroom_count * 5) + 3 (kitchen/laundry).");
			prompt.AppendLine("       * If ID: plumb_scope contains 'Само извеждане', Quantity = 3.");
			prompt.AppendLine("   - Riser Replace (PLMB-RISER-REPLACE): Quantity = 1 per bathroom ONLY if ID: plumb_riser contains 'Да, искам подмяна'.");
			prompt.AppendLine("   - Toilets (PLMB-WC-STD vs PLMB-WC-BUILTIN): Quantity = ID: global_bathroom_count. Map based on ID: plumb_wc_type.");
			prompt.AppendLine("   - Shower (PLMB-SHOWER-CABIN vs PLMB-SHOWER-FIXTURE): Quantity = ID: global_bathroom_count. Map based on ID: plumb_shower_type.");
			prompt.AppendLine();
			prompt.AppendLine("4. --- PAINTING (PANT) ---");
			prompt.AppendLine("   - Surface Area Calculation: Wall & Ceiling Area = (ID: global_total_sqm * 2.5).");
			prompt.AppendLine("   - Scope (PANT-PRIMER & PANT-PAINT-WHITE/COLOR): Quantity = Surface Area.");
			prompt.AppendLine("   - Spackling (PANT-SPACKLE-STD): Quantity = Surface Area ONLY if ID: paint_scope contains 'Стандартен ремонт' or 'Сваляне на тапети'.");
			prompt.AppendLine("   - Q5 Finish (PANT-SPACKLE-Q5): Quantity = Surface Area ONLY if ID: paint_finish_level contains 'Перфектно гладко (Q5)'.");
			prompt.AppendLine("   - Wallpaper Removal (PANT-WALLPAPER-REMOVE): Quantity = Surface Area ONLY if ID: paint_scope contains 'Сваляне на тапети'.");
			prompt.AppendLine("   - Doors/Trim (PANT-TRIM): Quantity = ID: paint_trim_doors_count.");
			prompt.AppendLine();
			prompt.AppendLine("5. --- DEMOLITION (DEMO) ---");
			prompt.AppendLine("   - Bath Demo (DEMO-BATH-FULL): Quantity = ID: global_bathroom_count ONLY if ID: demo_what contains 'Цяла баня'.");
			prompt.AppendLine("   - Wall Demo (DEMO-WALL-BRICK/CONC): Quantity = (ID: global_total_sqm * 0.2) if mentioned in demo_what.");
			prompt.AppendLine("   - Floor Demo (DEMO-FLOOR-TILE): Quantity = (ID: global_total_sqm * 0.3) if mentioned in demo_what.");
			prompt.AppendLine("   - Disposal (DEMO-DISPOSAL): Quantity = 1 if ID: demo_disposal contains 'Да, искам контейнер'.");
			prompt.AppendLine();
			prompt.AppendLine("6. --- DRYWALL (DRYW) ---");
			prompt.AppendLine("   - Area Proxy: If ID: drywall_rooms is 'В 1 стая' (20sqm), 'В 2-3 стаи' (50sqm), 'В целия обект' (global_total_sqm).");
			prompt.AppendLine("   - Ceiling (DRYW-CEILING-STD): Quantity = Area Proxy.");
			prompt.AppendLine("   - Walls (DRYW-WALL-PARTITION/LINING): Quantity = Area Proxy * 2.5.");
			prompt.AppendLine("   - Insulation (DRYW-INSULATION): Quantity = Area Proxy ONLY if ID: drywall_insulation contains 'Да'.");
			prompt.AppendLine("   - Boxes (DRYW-BOX): Quantity = ID: global_bathroom_count * 3 meters.");
			prompt.AppendLine();
			prompt.AppendLine("7. --- TILING (TILE) ---");
			prompt.AppendLine("   - Bath Area: Floor & Wall = ID: global_bathroom_count * 25 sqm.");
			prompt.AppendLine("   - Kitchen Area: Floor = 10 sqm.");
			prompt.AppendLine("   - Hallway/Other: Floor = ID: global_total_sqm * 0.3.");
			prompt.AppendLine("   - Tiling (TILE-STD/LARGE): Quantity = sum of chosen areas in ID: tile_rooms.");
			prompt.AppendLine("   - Laminate (TILE-LAMINATE): Quantity = ID: global_total_sqm * 0.7 if chosen.");
			prompt.AppendLine("   - Prep (TILE-PREP-LEVEL/HYDRO): Quantity = Area mapped to Tiling.");
			prompt.AppendLine();
			prompt.AppendLine("8. --- MICROCEMENT (MICRO) ---");
			prompt.AppendLine("   - Bath (MICRO-BATH): Quantity = ID: global_bathroom_count * 25 sqm if ID: mico_area contains 'Баня'.");
			prompt.AppendLine("   - Standard (MICRO-STD): Quantity = ID: global_total_sqm * 0.5 if ID: mico_area contains 'Сухи зони'.");
			prompt.AppendLine();
